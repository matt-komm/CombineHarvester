...
def test_ratios(k_obs, k_pre):
	...
	for i in range(len(k_obs)):
		...
		print('>> Case {} pre/obs = {} / {} = {:.3f}'.format(i, k_pre[i], k_obs[i], k_pre[i]/k_obs[i]))
		...
		print('Poisson ratio uncertainty CI68: {}'.format(ppois68))
		print('Poisson ratio uncertainty CI95: {}'.format(ppois95))
		...
		print('Systematic residual: {:.2f}'.format(res))
		
		
		# Single sample Poisson tail integral
		ppois = poisson_tail(k1=k_pre[i], k2=k_obs[i])
		print('Poisson tail p = {:0.3E} ({:0.2f} sigmas)'.format(ppois, p2zscore(ppois)))

		# Error propagation
		errAB = prodratio_eprop(A=k_pre[i], sigmaA=s_pre[i], B=k_obs[i], sigmaB=s_obs[i])
		PRE_over_OBS = k_pre[i] / k_obs[i]
		print(' error propagated rel.uncertainty on the ratio: err / (pre/obs) = {:.3f}'.format(errAB / PRE_over_OBS))
